...
#ANSIBLE_HASHI_VAULT_PROXY={"http": "socks5h://unixsocket/tmp/mysock.sock", "https": "socks5h://unixsocket/tmp/mysock.sock"}
#ANSIBLE_HASHI_VAULT_PROXY={"http": "socks5h://localhost:1234", "https": "socks5h://localhost:1234"}
ANSIBLE_HASHI_VAULT_PROXY=None

if os.getenv('VAULT_ADDR') is not None:
    ANSIBLE_HASHI_VAULT_ADDR = os.environ['VAULT_ADDR']
#if os.getenv('VAULT_PROXY') is not None: ANSIBLE_HASHI_VAULT_PROXY=json.loads(os.environ['VAULT_PROXY'])
        ...
        if self.auth_method and self.auth_method != 'token':
            try:
                if self.namespace is not None:
                    self.client = hvac.Client(url=self.url, verify=self.verify, namespace=self.namespace,proxies=self.proxies)
                else:
                    self.client = hvac.Client(url=self.url, verify=self.verify,proxies=self.proxies)
            ...
            if self.namespace is not None:
                self.client = hvac.Client(url=self.url, token=self.token, verify=self.verify, namespace=self.namespace,proxies=self.proxies)
            else:
                self.client = hvac.Client(url=self.url, token=self.token, verify=self.verify,proxies=self.proxies)
